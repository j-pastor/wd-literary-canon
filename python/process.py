...
with open(prefix+"wikidata-properties.csv") as data:
    for reg in csv.DictReader(data):
        ...
        prop_stat[p]={"label":reg["propertyLabel"],"num":0,"isID":False,"descripcion":reg["propertyDescription"],"total_use":0,"top_wikis":0,"top_props":0,"top_words":0,"top_wikirank":0}
...
with open(prefix+"wikidata-items-properties.csv","r") as data:
    for reg in csv.DictReader(data):
        ...
        if q in item_stat:
            ...
            item_stat[q]={}
            if prop_stat[p]["isID"]==True:
                item_stat[q]={"item":q, "title":"","title_bis":"","date":0,"date_type":0,"precision_date":0, "langs":{}, "wikis":{}, "props_id":{}, "props_non_id":{}, "num_id":1,"num_non_id":0, "num_langs":0, "num_wikis":0, "total_num_words":0, "wikirank":0}
            else:
                item_stat[q]={"item":q, "title":"","title_bis":"","date":0,"date_type":0,"precision_date":0, "langs":{}, "wikis":{}, "props_id":{}, "props_non_id":{}, "num_id":0,"num_non_id":1, "num_langs":0, "num_wikis":0, "total_num_words":0, "wikirank":0}
...
with open(prefix+"wikidata-items-languages.csv") as data:
    for reg in csv.DictReader(data):
        ...
        if code in lang_stat:
            ...
            lang_stat[code]={"num_items":1,"total_wikis":0,"total_id_props":0,"total_props":0,"total_words":0,"total_wikirank":0}
...
for item in item_stat:
    ...
    item_stat[item]["wikirank"]=log(1+nwi)+log(1+np)+log(1+nwo)
...
sorted_wikirank=dict(sorted(item_stat.items(), reverse=True, key=lambda item: item[1]["wikirank"]))
...
for i in range(len(selected_items)):
    ...
    for p in props:
        ...
        props=selected_wikirank[i][1]["props_non_id"]
    for p in props:
        prop_stat[p]["top_wikirank"]+=1
...
for i in range(len(selected_items)):
    ...
    for p in props:
        prop_stat[p]["top_words"]+=1
        props=selected_wikirank[i][1]["props_id"]
    for p in props:
        prop_stat[p]["top_wikirank"]+=1
...
with open(prefix+"results/results-top-items-properties.csv","w") as file:
    print('"Property","Property Label","is ID","Total use","Percent by total","Top by wikis","Percent by wikis","Top by statements","Percent by statements","Top by words","Percent by words","Top by WikiRank","Percet by WikiRank"',file=file)
    for prop in sorted_prop_stat:
        print('"'+prop+'","'+prop_stat[prop]["label"]+'","'+str(prop_stat[prop]["isID"])+'",'+str(prop_stat[prop]["total_use"])+','+str(round(prop_stat[prop]["total_use"]/total*100,2))+','+str(prop_stat[prop]["top_wikis"])+','+str(round(prop_stat[prop]["top_wikis"]/top*100,2))+','+str(prop_stat[prop]["top_props"])+','+str(round(prop_stat[prop]["top_props"]/top*100,2))+','+str(prop_stat[prop]["top_words"])+','+str(round(prop_stat[prop]["top_words"]/top*100,2))+','+str(prop_stat[prop]["top_wikirank"])+','+str(round(prop_stat[prop]["top_wikirank"]/top*100,2)),file=file)
    file.close()
...
with open(prefix+"results/results-matrix-lang-wikis-wikirank.csv","w") as file:
    print('"lang"',file=file,end='')
    for data_1 in sorted_wiki_stat:
        print(',"'+data_1+'"',file=file,end='')
    print("\n",file=file,end='')
    for data_1 in sorted_lang_stat:
        print('"'+data_1+'"',file=file, end='')
        for data_2 in sorted_wiki_stat:
            print(','+str(matrix_langs_wikis_wikirank[data_1][data_2]),file=file, end='')
        print("\n",file=file,end='')
    file.close()
...
with open(prefix+"results/results-items-description.csv","w") as file:
    print("item,language,all_languages,title,title_bis,date,date_type,precision_date,num_wikis,num_non_id_props,num_id_props,total_num_words,WikiRank",file=file)
    for item in sorted_item_stat:
            ...
            for lang in sorted_item_stat[item]["langs"]:
                print(item+','+lang+',"'+all_languages+'","'+sorted_item_stat[item]["title"]+'","'+sorted_item_stat[item]["title_bis"]+'",'+
                    ...
                    str(sorted_item_stat[item]["total_num_words"])+","+str(sorted_item_stat[item]["wikirank"]),file=file)
                ...
                lang_stat[lang]["total_wikirank"]+=sorted_item_stat[item]["wikirank"]
    file.close()
...
with open(prefix+"results/results-languages-description.csv","w") as file:
    print("Language code,Total items,Wikis average,Statements average,ID Statements average,Words average,WikiRank average",file=file)
    for lang in lang_stat:
        ...
        avg_wikirank=lang_stat[lang]["total_wikirank"]/tot_items
        print(lang+","+str(tot_items)+","+str(avg_wikis)+","+str(avg_props)+","+str(avg_id_props)+","+str(avg_words)+","+str(avg_wikirank),file=file)
    file.close()
...
for item in sorted_wikirank:
    list_by_wikirank.append(item)
    values_by_wikirank.append(sorted_item_stat[item]["wikirank"])

with open(prefix+"results/results-item-rankings.csv","w") as file:
    print("Ranking,Item by wikis,Title by wikis,Value by wikis,Item by props,Title by props,Value by props,Item by words,Title by words,Value by words,Item by WikiRank,Title by WikiRank,Value by WikiRank",file=file)

    for i in range(len(list_by_nwikis)):
        i_nwikis=list_by_nwikis[i]
        i_nprops=list_by_nprops[i]
        i_nwords=list_by_nwords[i]
        i_wikirank=list_by_wikirank[i]
        ...
        v_wikirank=values_by_wikirank[i]
        ...
        print(","+i_wikirank+",\""+item_stat[i_wikirank]["title"]+"\","+str(v_wikirank),file=file)
